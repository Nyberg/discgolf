 ...
 * =========================================================== */
!function(d){var b="assets/php/twitter.php",c="icon-twitter icon-large";d(".tweet").each(function(){d.ajax({dataType:"json",url:b,context:this,success:function(g){var h=d(this),e=h.data("icon")||c,f;f='<ul class="tweetList">';d.each(g,function(j,k){var l=k.text.replace(/((https?|s?ftp|ssh)\:\/\/[^"\s\<\>]*[^.,;'">\:\s\<\>\)\]\!])/g,function(i){return'<a href="'+i+'">'+i+"</a>"}).replace(/\B@([_a-z0-9]+)/ig,function(i){return'<a href="http://twitter.com/'+i.substring(1)+'">'+i+"</a>"});f+='<li class="tweet"><em class="'+e+'"></em><p class="tweetText">'+l+'<br><small class="tweetTime">'+a(k.created_at)+"</small></p></li>"});f+="</ul>";h.append(f)}})});function a(g){var f=g.split(" ");g=f[1]+" "+f[2]+", "+f[5]+" "+f[3];var e=Date.parse(g);var h=(arguments.length>1)?arguments[1]:new Date();var i=parseInt((h.getTime()-e)/1000);i=i+(h.getTimezoneOffset()*60);if(i<60){return"less than a minute ago"}else{if(i<120){return"about a minute ago"}else{if(i<(60*60)){return(parseInt(i/60)).toString()+" minutes ago"}else{if(i<(120*60)){return"about an hour ago"}else{if(i<(24*60*60)){return"about "+(parseInt(i/3600)).toString()+" hours ago"}else{if(i<(48*60*60)){return"1 day ago"}else{return(parseInt(i/86400)).toString()+" days ago"}}}}}}}}(window.jQuery);
/* ===========================================================
 * jQuery script for Dropper Skin Custom Inputs
 * =========================================================== */
!function(a){a(function(){var c="icon-ok";a(".checkbox").each(function(){var g=a(this),f=g.children("input:checkbox"),e=a('<span class="ctrl-overlay"/>').appendTo(g);if(f[0].checked){if(!f[0].disabled){g.addClass("checked")}e.addClass(c)}});a(".checkbox").on("click",function(){var g=a(this),e=g.children("input:checkbox"),f=g.children(".ctrl-overlay");if(e[0].disabled){return}g[e[0].checked?"addClass":"removeClass"]("checked");f[e[0].checked?"addClass":"removeClass"](c)});var d="icon-circle-blank",b="icon-circle";a(".radio").each(function(){var g=a(this),f=g.children("input:radio"),e=a('<span class="ctrl-overlay"/>').appendTo(g);e.addClass("icon-circle");if(f[0].checked){if(!f[0].disabled){g.addClass("checked")}e.addClass(d).removeClass(b)}});a(".radio").on("click",function(){var g=a(this),f=g.children("input:radio"),e=g.children(".ctrl-overlay");if(f[0].disabled){return}a("input:radio[name="+f.attr("name")+"]").not(":disabled").siblings(".ctrl-overlay").removeClass(d).addClass(b).parent().removeClass("checked");if(f[0].checked){g.addClass("checked");e.removeClass(b).addClass(d)}});a('select[data-toggle="dropdown-select"]').each(function(){var h=a(this),f,g,e="";h.hide();g=h.find("option").map(function(){return a('<li><a href="#" rel="'+a(this).attr("value")+'">'+a(this).text()+"</a></li>").get(0)});e+=h.data("size")?" btn-"+h.data("size"):"";e+=h.data("style")?" btn-"+h.data("style"):"";f=a('<div class="btn-group"/>').append(a('<button class="btn dropdown-toggle dropdown-select'+e+'" data-toggle="dropdown">'+h.find("option:first").text()+' <span class="caret"></span></button>')).append(a('<ul class="dropdown-menu"/>').append(g)).appendTo(h.parent()).find(".dropdown-select");f.width(h.width());f.siblings(".dropdown-menu").on("click","li > a",function(i){i.preventDefault();f.contents().filter(function(){return this.nodeType===3})[0].nodeValue=a(this).text()+" ";h.val(a(this).attr("rel")).change()})})})}(window.jQuery);
